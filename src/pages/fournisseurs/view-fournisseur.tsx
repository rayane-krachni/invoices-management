...
import { FournisseurModel } from "./update-fournisseur";

interface ViewFournisseurModalProps {
  isOpen: boolean;
  fornisseur: FournisseurModel | null;
  onClose: () => void;
}

export const ViewFournisseurModal: React.FC<ViewFournisseurModalProps> = ({
  isOpen,
  fornisseur,
  onClose,
}) => {
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto p-8 relative animate-fade-in">
        ...
        <div className="flex flex-col items-center mb-2">
          <FaBuilding className="text-blue-600 text-6xl mb-2" />
          <h2 className="text-3xl font-bold text-blue-700 mb-1">Détails du Fournisseur</h2>
          <p className="text-gray-500 text-sm">Informations enregistrées dans le système</p>
        </div>

        {/* Details Section */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {Object.entries(fornisseur)
...
export default ViewFournisseurModal;
